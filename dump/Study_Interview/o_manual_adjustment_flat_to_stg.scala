...
def colType(datatype:String) = {
  ...
  val tableName = "g00103.manual_adjustment_flatfile_to_stg"
...
("formula_driven_transaction_type","string"),
("channel1","string"),
...
order_number1,
sales_account_no,
formula_driven_transaction_type,
channel1,
adjustment_type,
us_cm_value,
local_currency1,
...
//Code to read the CSV Indirect_ORDERS.txt from HDFS
    ...
    val indirectOrders_ds = readCSV(indirectOrders_path,indirectOrders_schema,false,false).as[String]  //Indirect_BACKLOG_temp
...
for ( fileName <-fileNameArray_orders ){
    val file_path = csvfilePath + fileName
    val temp_df = readCSV(file_path,schema_orders,false,true)
    temp_df.createOrReplaceTempView("temp_df")
...
Order_Number,
Purchase_Order_No,
to_date(from_unixtime(unix_timestamp(concat(substr(Order_Booked_Date,9,2),'-',substr(Order_Booked_Date,5,3),'-',substr(Order_Booked_Date,25,4)),'dd-MMM-yyyy'))),
Management_Entity,
Formula_Driven_Tier5,
Product_Class,
Transaction_Type,
...
to_date(from_unixtime(unix_timestamp(concat(substr(Customer_Request_Date,9,2),'-',substr(Customer_Request_Date,5,3),'-',substr(Customer_Request_Date,25,4)),'dd-MMM-yyyy'))),
to_date(from_unixtime(unix_timestamp(concat(substr(T_D_only_Start_Date,9,2),'-',substr(T_D_only_Start_Date,5,3),'-',substr(T_D_only_Start_Date,25,4)),'dd-MMM-yyyy'))),
to_date(from_unixtime(unix_timestamp(concat(substr(T_D_only_End_Date,9,2),'-',substr(T_D_only_End_Date,5,3),'-',substr(T_D_only_End_Date,25,4)),'dd-MMM-yyyy'))),
Customer_Sector,
Sales_Person,
Reason_for_manual_adjustment,
...
AND LENGTH(Customer_Request_Date) =28
AND LENGTH(Order_Booked_Date) = 28""")
