...
def generateSeq(counter: Int) =  {
  ...
  val tableName = "IQP_SC_ADJUSTMENTS_STG"
...
val sc_backlog_exploded_df = sc_backlog_exploded.withColumn("net_amount", sc_backlog_exploded("temp._1") ).withColumn("key_code",sc_backlog_exploded("temp._1") ).drop("temp" )
...
(SELECT max(IQP_DATE_DIM.DATE_VALUE) as DATE_VALUE, 
...
where sc_backlog.net_amount is not  null and target_lkp.order_number is  null""")



//SC_BACKLOG MULTIPLICATION SQL PROCEDURE

var multiplication_df=spark.sql("""SELECT a.*,datediff(backlog_end_date,process_date) as counter FROM g00103.IQP_SC_ADJUSTMENTS_STG_BACKLOG a WHERE RECORD_CREATION_DATE 
=(SELECT  MAX(RECORD_CREATION_DATE) FROM g00103.IQP_SC_ADJUSTMENTS_STG_BACKLOG)""")

val udf_generateSeq = udf(generateSeq _)

val multiplicated_df= multiplication_df.withColumn("exploded_col", explode(udf_generateSeq(multiplication_df("counter")))).withColumn("metric_adjusted",multiplication_df("metric_adjusted") * multiplication_df("exploded_col"))
...
spark.udf.register("getMonth", (some_date:String) => { 
val years = some_date.substring(24,4)
val months = some_date.substring(4,3).toUpperCase
val dates = some_date.substring(8,2)

val res1 = months match {
case "JAN" => "01"
case "FEB" => "02"
case "MAR" => "03"
case "APR" => "04"
...
val f_res = years+ "-"+ res1+ "-"+ dates
f_res
})


spark.udf.register("getDate", (some_date:String) => { 
val years = some_date.substring(7,4)
val dates = some_date.substring(1,2)
val months = some_date.substring(4,2)
val f_res =years+ "-" + months + "-" + dates
f_res
})
...
a.Purchase_Order_No,
case 
when LENGTH(a.Order_Booked_Date)>10 then
to_date(getMonth(a.Order_Booked_Date))

else

to_date(getDate(a.Order_Booked_Date))  end,
a.Management_Entity,
...
else case when LENGTH(a.Customer_Request_Date)>10 then
to_date(getMonth(a.Customer_Request_Date))

else

to_date(getDate(a.Customer_Request_Date)) end end,
NULL,
NULL,
a.Customer_Sector,
a.Sales_Person,
...
a.Order_Number,
a.Purchase_Order_No,
case when LENGTH(a.Order_Booked_Date)>10 then
to_date(getMonth(a.Order_Booked_Date))
...
group by FISCAL_WEEK,FISCAL_YEAR) b on (a.Week_No_FW = b.FISCAL_WEEK and a.FISCAL_YEAR = b.FISCAL_YEAR)
left join (SELECT
max(IQP_DATE_DIM.DATE_KEY) as DATE_KEY, 
max(IQP_DATE_DIM.DATE_VALUE) as DATE_VALUE, 
max(IQP_DATE_DIM.FISCAL_YEAR) as FISCAL_YEAR, 
max(IQP_DATE_DIM.FISCAL_YEAR_NAME) as FISCAL_YEAR_NAME, 
IQP_DATE_DIM.FISCAL_QUARTER as FISCAL_QUARTER, 
max(IQP_DATE_DIM.FISCAL_WEEK) as FISCAL_WEEK 
 FROM g00103.IQP_DATE_DIM IQP_DATE_DIM
