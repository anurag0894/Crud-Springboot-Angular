...
object o_legacy_invoice_mgmt_fact
{
...
'Pulse Reporting' as REPORT_PROCESS1,
...
FROM g00103.IQP_LEGACY_SALES_COMMON_STG IQP_LEGACY_SALES_COMMON_STG left join
...
-- WHERE IQP_LEGACY_SALES_COMMON_STG.CURRENT_FLAG = 'Y' """).createOrReplaceTempView("mytempTable") 

spark.sql("cache table mytempTable")
...
cw_go_schedule,
cw_family_id,
legProductGroup(product_number,ORGANIZATION_NAME,PRODUCTCLASS,PRODUCTCLASSGROUP,DATA_SOURCE,MANAGEMENT_ENTITY) as product_group,
...
a.PRODUCTCLASS,
legProductPlatform(product_number,ORGANIZATION_NAME,PRODUCTCLASS,PRODUCTCLASSGROUP,DATA_SOURCE,MANAGEMENT_ENTITY,product_platform,"") as product_platform ,
...
legBusinessSegment(product_number,ORGANIZATION_NAME,PRODUCTCLASS,PRODUCTCLASSGROUP,TIER2,TIER3,legProductPlatform(product_number,ORGANIZATION_NAME,PRODUCTCLASS,PRODUCTCLASSGROUP,DATA_SOURCE,MANAGEMENT_ENTITY,product_platform,"")) as business_segment,
legOrderTypePulse(TIER2,TIER3,DATA_SOURCE,order_type,legProductPlatform(product_number,ORGANIZATION_NAME,PRODUCTCLASS,PRODUCTCLASSGROUP,DATA_SOURCE,MANAGEMENT_ENTITY,product_platform,"")) as order_type_pulse,
item_type_code,
producing_plant,
...
""").createOrReplaceTempView("to_write") 

spark.sql("insert into g00103.iqp_invoice_mgmt_fact partition(data_source) select * from to_write")
 
//legacy flag update for non-bravo data

val bravo_df=spark.sql("""select *
from g00103.IQP_LEGACY_SALES_COMMON_STG 
where data_source='BRAVO'
""")

//flag from Y to N of daily data

val non_bravo_df=spark.sql("""select order_requisition_number,
line_number,
invoice_number,
invoice_line_number,
catlog_number,
...
//union of bravo and non_bravo data

val main=bravo_df.union(non_bravo_df)
main.createOrReplaceTempView("main_vw")

//creating temp table for updated data

spark.sql("create table g00103.tmp_legacy_invoice_sales as select * from main_vw")
spark.sql("drop table g00103.IQP_LEGACY_SALES_COMMON_STG")

//altering table name from temp to legacy common stage
spark.sql("alter table g00103.tmp_legacy_invoice_sales  rename to g00103.IQP_LEGACY_SALES_COMMON_STG")
