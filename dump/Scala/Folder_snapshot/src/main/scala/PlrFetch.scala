...
val sc = spark.sparkContext
import spark.implicits._
val df_tbl_catalog_list_plant = spark.sql(""" select max(productline) as product_line,max(product_segment) as product_segment,max(productclass) as product_class, concat(trim(item_number),',',trim(PLANTCODE)) as key from g00103.IQP_LEGACY_CATALOG_LIST  where productline !='NAVL' group by item_number,PLANTCODE""")
...
val df_tbl_catalog_list_platform = spark.sql(""" select max(productline) as product_line,max(product_segment) as product_segment, max(productclass) as product_class, trim(item_number) as item_number from g00103.IQP_LEGACY_CATALOG_LIST  where productline !='NAVL' group by item_number""")
...
val product_platform_pd = 
   ...
   else if(pnl =="CLEARWATER" || pnl =="SOWO") product_platform_plr
   else if(tier2 == "CAPACITORS" && tier3 == "CAPACITORS") product_platform_plr
   else if(tier2 == "CAPACITORS" && tier3 == "NETWORK REGS & TRANS") product_platform_plr
   else if(tier2 == "ACS" ) product_platform_plr
   else if(tier2 == "XD PRIMARY EQUIPMENT") product_platform_plr
   else subpnl
...
val result = if (tier2 == "GRID AUTOMATION") business_segment_ga  else ""
...
def getOrderP(broadcastSubBusiness:Broadcast[Map[(String,String),String]]) = {
val getOrderPulse = (tier2:String,tier3:String,productPlatform:String) => { 

val product_Platform  =  Option(productPlatform).getOrElse("").toUpperCase 
val ordertypepulse  = if( product_Platform=="PACKAGED SOLUTIONS")  "PROJECT_ORDERS" else "PRODUCT_ORDERS"
